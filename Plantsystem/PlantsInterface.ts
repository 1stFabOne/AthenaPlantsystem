...
export async function buildPlant(plantOwner: string, posX: number, posY: number, posZ: number, state: string, remainingMinutes: number, isHarvestable: any) {
    const newDocument = {
...
export async function loadAllExistingPlants() {
    ...
    allPlants.forEach(plant => {
        ...
        TextLabelController.append({
            ...
            data: `~n~~w~State: ${plant.data.state}~n~~w~Time: ~g~${plant.data.remainingMinutes} ~w~minutes`,
            maxDistance: 10
        });
...
async function harvestPot(player: alt.Player) {
    ...
    allPlants.forEach(async plants => {
        if (distance2d(player.pos, plants.position) <= 1) {
            ...
            playerFuncs.emit.scenario(player, "WORLD_HUMAN_GARDENER_PLANT", 5000);
            alt.setTimeout(() => {
